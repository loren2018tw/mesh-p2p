...
- [x] 6.1 測試大檔案（>1 GB）下載是否能直接寫入檔案系統而不 OOM
- [x] 6.2 測試多檔案同時下載時 `FileSystemChunkStore` 各自獨立 write queue 不相互衝突
- [x] 6.3 測試不支援瀏覽器（Firefox）載入頁面時是否正確顯示不支援提示
- [x] 6.4 測試頁面重新載入後能否自動恢復先前授權的目錄 handle
- [x] 6.5 測試下載過程中停止（`stopTransfer`）後檔案是否被正確清理
